    ...
    methods(Static)
        function pool = setup_parallel_pool()
            ...
            if isempty(gcp('nocreate'))
                ...
                logger.log_message('info', sprintf('检测到%d个逻辑处理器', numLogicalProcessors));
                ...
                logger.log_message('info', sprintf('创建并行池，使用%d个worker，每个worker使用%d个线程', poolSize, c.NumThreads));
            else
                pool = gcp('nocreate');
                logger.log_message('info', '使用现有并行池');
            end
        end
    end
end
